...
<header id="mainHeader" class="bg-white shadow-sm sticky top-0 z-50">
    ...
    <div class="max-w-7xl mx-auto px-4 py-3">
        <div class="flex items-center justify-between gap-4">
            ...
            <div class="hidden md:flex flex-1 max-w-xl mx-8 items-center gap-3">
                <div class="relative w-full">
                    <input type="text" placeholder="Search products..." class="input-field w-full border border-gray-200 rounded-full py-2.5 px-5 pr-12 focus:outline-none focus:border-primary">
                    <button class="absolute right-1 top-1 bg-primary text-white rounded-full w-9 h-9 flex items-center justify-center hover:bg-primary-dark transition">
                        <i class="fas fa-search text-sm"></i>
                    </button>
                </div>
                <a href="{{ route('home') }}" class="bg-primary text-white px-4 py-2.5 rounded-full text-sm font-semibold whitespace-nowrap hover:bg-primary-dark transition">Start Shopping</a>
            </div>
            <div class="flex items-center gap-4">
                <a href="{{ route('support') }}" class="hidden md:flex text-gray-600 hover:text-primary transition items-center gap-1">
                ...
                <div class="hidden md:flex items-center gap-3">
                    @auth
                        <a href="{{ route('user.dashboard') }}" class="text-gray-600 hover:text-primary transition flex items-center gap-1">
                            <i class="fas fa-user"></i>
                            <span class="text-sm font-medium">My Account</span>
                        </a>
                    @else
                        <a href="{{ route('login') }}" class="text-gray-600 hover:text-primary transition flex items-center gap-1">
                            <i class="fas fa-user"></i>
